...
const AllMilk = () => {
  ...
  const allMilks = async () => {
    try {
      const milksData = await axios.get(`http://localhost:5000/av1/allmilks/${ownerId}`);
      dispatch(addAllMilk(milksData.data));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const filteredMilkData = [];

  data?.milkWithCow?.cowsData?.forEach((cow) => {
    const filtered = cow.milkData?.filter((milk) =>
      milk.cowId?.toString().toLowerCase().includes(searchTerm)
    );
    if (filtered?.length) filteredMilkData.push(...filtered);
  });

  const totalPages = Math.ceil(filteredMilkData.length / itemsPerPage);
  const indexOfLast = currentPage * itemsPerPage;
  const indexOfFirst = indexOfLast - itemsPerPage;
  const currentMilkItems = filteredMilkData.slice(indexOfFirst, indexOfLast);
  ...
  const deleteMilk = async () => {
    await axios.delete(`http://localhost:5000/av1/deletemilk/${milk._id}`);
    toast.success("Milk is Deleted");
    setModal(false);
    allMilks();
  };
    ...
    <>
      ...
      <div className="container-fluid" style={{ backgroundColor: "rgb(33, 33, 33)", minHeight: "100vh" }}>
        <div className="container">
          ...
          <div className="row row-cols-1 row-cols-lg-5 row-cols-md-4 g-4 d-flex justify-content-center align-items-center">
            {searchTerm.length === 0 ? (
              <span className='text-primary fs-3 text-center'>Search Cow Milk</span>
            ) : filteredMilkData.length === 0 ? (
              <span className='text-danger fs-3 text-center'>Milk Not Found</span>
            ) : (
              currentMilkItems.map((value, index) => (
                <div className='col m-2' key={index} onClick={() => handleClick(value)} style={{ cursor: 'pointer' }}>
                  <StyledWrapper>
                    <div className="card">
                      <p>CowId: {value.cowId}</p>
            ...
            <div className="w-100 d-flex flex-column ">
              <div className="row d-flex justify-content-center">
                <div className="col-12 col-md-9 col-lg-6 position-relative rounded-5 shadow" style={{ height: "50vh", backgroundColor: "rgb(90, 90, 90)" }}>
                  ...
                  <div className="d-flex justify-content-around align-items-center mt-5">
                    <div onClick={updateMilk}><button className='fs-5 btn btn-info'>Update</button></div>
                    <div className="mb-2" onClick={deleteMilk}><button className='fs-1' style={{ backgroundColor: "inherit" }}><MdDelete /></button></div>
                  </div>
                </div>
              </div>
            </div>
